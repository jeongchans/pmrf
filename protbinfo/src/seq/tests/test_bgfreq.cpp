...
TEST_F(BgFreqTest, test_bgfreq) {
    ...
    for (std::map<char, FloatType>::iterator pos = freq.begin(); pos != freq.end(); ++pos) pos->second /= sum(f);
    f /= sum(f);
    ...
    for (int i = 0; i < (int)s.size(); ++i) {
        EXPECT_EQ(f(i), bgfreq.get_value(s[i]));
    }
}
...
TEST_F(BgFreqTest, test_robinson) {
    ...
    EXPECT_FLOAT_EQ(0.02199, robinson.get_value('H'));
    EXPECT_FLOAT_EQ(0.07120, robinson.get_value('S'));
}
...
TEST_F(BgFreqTest, test_altschul) {
    ...
    EXPECT_FLOAT_EQ(0.02200, altschul.get_value('H'));
    EXPECT_FLOAT_EQ(0.06800, altschul.get_value('S'));
}
