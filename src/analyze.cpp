...
int MRFModelAnalyzer::stat(const string& mrf_filename) {
    string delim = " ";
    MRF model = read_mrf(mrf_filename);
    if (stat_opt.mode == Stat::STATMODE_PAIR) {
        PairScoreVector scores = calc_pair_score(model);
        if (stat_opt.corr == Stat::STATCORR_APC) scores = correct_apc_pair_score(scores);
        else if (stat_opt.corr == Stat::STATCORR_NCPS) scores = correct_ncps_pair_score(scores);
        calc_zscore(scores);
    ...
    } else if (stat_opt.mode == Stat::STATMODE_POS) {
